...
def filter_data(data, group_size=250):
    ...
    for item in data:
        ...
        if gender == "M" and tempo == "high":
            groups["male_fast"].append(item)
        elif gender == "F" and tempo == "high":
            groups["female_fast"].append(item)
        elif gender == "M" and tempo == "low":
            groups["male_slow"].append(item)
        elif gender == "F" and tempo == "low":
    ...
    for single_data in result:
        ...
        sample = dict(
            ...
            question="How do you feel about the current pace of the speech? Please respond with 'rapid' or 'slow.'",
            answer=change_label[single_data["tempo"]],
...
if __name__ == "__main__":
    ...
    # 保存筛选后的数据
    dump_json(json_filename="Q_%s.json" % "Speech Rate".replace(" ", "_"),
              dump_data=new_list)